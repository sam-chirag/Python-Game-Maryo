...
class dragon1:
    ...
    global firerect1, imagerect1, Canvas
    up = False
    down = True
    velocity = 25
    
    def __init__(self):
        self.image1 = load_image('dragon.png')
        self.imagerect1 = self.image1.get_rect()
        self.imagerect1.right = window_width
        self.imagerect1.top = window_height/2    
        
        

    def update(self):
        
        if (self.imagerect1.top < cactusrect.bottom):
            self.up = False
            self.down = True

        if (self.imagerect1.bottom > firerect.top):
            self.up = True
            self.down = False
            
        if (self.down):
            self.imagerect1.bottom += self.velocity

        if (self.up):
            self.imagerect1.top -= self.velocity

        Canvas.blit(self.image1, self.imagerect1)

    def return_height(self):

        h = self.imagerect1.top
        return h
...
class flames1:
    ...
    def __init__(self):
        self.image1 = load_image('fireball.png')
        self.imagerect1 = self.image1.get_rect()
        self.height = Dragon1.return_height() + 20
        self.surface = pygame.transform.scale(self.image1, (20,20))
        self.imagerect1 = pygame.Rect(window_width - 106, self.height, 20, 20)

    def update(self):
            self.imagerect1.left -= self.flamespeed

    def collision(self):
        if self.imagerect1.left == 0:
            return True
        else:
            return False
...
gameover = pygame.mixer.Sound('last.wav')
...
while True:
    ...
    while True:     
        ...
        Canvas.blit(Dragon1.image1, Dragon1.imagerect1)# amendment done here 
